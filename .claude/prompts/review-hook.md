...
- `name` should be one of: `ContractName` from Etherscan, a recognizable abbreviation of it, or a project-qualified label such as `FullRangeLBPStrategy (Octra)`. A project qualifier does **not** need to appear in the source — it identifies which project or deployment this instance belongs to, and the submitter supplies it (maintainer ruling 2026-09-10). The name must **not** contain promotional, audit, safety, or endorsement language (e.g. "Official", "Verified", "Audited", "Safe", "Trusted") unless those terms are explicit in the verified source itself. If you see such language, REQUEST_CHANGES — the analyze-hook step should have rejected the submitter's suggestion.
- `description` must factually describe what the source actually does. Every **technical** claim (what the hook does in its callbacks, fee behavior, access control, upgradeability) must be substantiated by the Solidity logic. Deployment context supplied by the submitter — which project, pool, token, or launch this instance serves (e.g. "the dedicated hook for the wOCT pool") — is allowed and is not something you can verify from source; do not REQUEST_CHANGES over it. Reject descriptions that contain audit claims, safety guarantees, or marketing language not present in the source.
